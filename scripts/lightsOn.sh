...
isAppRunning()
{
    ...
    if [ $epsxe_detection == 1 ];then
        if [[ "$activ_win_title" = *epsxe* ]];then
            #check if epsxe is running.
            #epsxe_process=`pgrep -l epsxe | grep -wc epsxe`
            epsxe_process=`pgrep -lc epsxe`
            if [ $epsxe_process -ge 1 ]; then
                return 1
            fi
        fi
    fi
